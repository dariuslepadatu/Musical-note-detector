...
byte NoteV[13] = {8,   23,  40,  57,  76,  96, 116,
                  138, 162, 187, 213, 241, 255};  // data for note detection
                                                  // based on frequency
...
void setup() {
  ...
  Mic_pin = A0;  // change as per Microphone pin
}
...
const char* Tone_det() {
  ...
  if (sum2 - sum1 > 3) {
    ...
    // this code updates f_peaks array with 5 most dominent frequency in
    // descending order
    ...
    for (int i = 0; i < 5; i++) {
      ...
      while (j == 1) {
        ...
        k++;  // a note with max peaks (harmonic) with aplitude priority is
              // selected
        if (k > 15) {
          j = 0;
        }
    ...
    // Note print
    // if you need to use note value for some application, use of note number
    // recomendded where, 0=c;1=c#,2=D;3=D#;.. 11=B;
...
//Function----------------------------------------------//
// Documentation on
// EasyFFT:https://www.instructables.com/member/abhilash_patel/instructables/
...
float FFT(byte N, float Frequency) {
  ...
  for (int i = 0; i < data[o];
       i++)  // update input array as per bit reverse order
  {
    if (in_ps[i] < a) {
      out_r[i] = in[in_ps[i]];
    }
  ...
  {
    out_r[i] =
        sqrt((out_r[i] * out_r[i]) +
             (out_im[i] * out_im[i]));  // to  increase the speed delete sqrt
    out_im[i] = (i * Frequency) / data[o];
    /*
    Serial.print(out_im[i],2); Serial.print("Hz");
    Serial.print("\t");                            // uncomment to print
    freuency bin Serial.println(out_r[i]);
    */
  }
  ...
  for (int i = 0; i < 5;
       i++)  // updating f_peak array (global variable)with descending order
  {
